...
def load_kratos_yt( d, gamma=5./3., delete_file=False, fld_extra = {}, **kwargs ):
    ...
    filename = kwargs.get( "filename", "kratos_test" )
    if "filename" in kwargs:
        del kwargs[ "filename" ]
    enroll_mesh_tree( d )
    kratos_to_hdf5( d, filename=filename, fld_extra = fld_extra, gamma=gamma )
    ds = yt.load( "kratos_test.athdf", **kwargs )
    if delete_file:
        os.remove(f"{filename}.athdf")
    return ds

def kratos_to_hdf5( d, filename="kratos_test", fld_extra = {}, gamma=5./3. ):
    ...
    var_names_extra = np.array( [ i for i in fld_extra.keys( ) ], dtype=np.bytes_ )
